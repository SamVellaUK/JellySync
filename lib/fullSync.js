...
const { shouldSyncUser, resolveTargetUsername, resolveSameServerPartner } = require('./config');

/**
 * Sync a single item to a target server for a given target username
 */
async function syncItemToTarget(sourceItem, targetUsername, targetServer) {
  try {
    // Get target user
    const targetUserId = await getUserByName(targetServer, targetUsername);
    if (!targetUserId) {
      console.error(`[FULL-SYNC] User ${targetUsername} not found on ${targetServer.name}`);
...
async function performFullSync(config) {
  ...
  // Get target servers: all except master, plus master itself if it has same-server user mappings
  const otherServers = config.subscribers.filter(s => s.name.toLowerCase() !== config.masterServer.toLowerCase());
  const masterHasSelfMapping = !!(masterServer.userMap && Object.keys(masterServer.userMap).length > 0);
  const targetServers = masterHasSelfMapping ? [...otherServers, masterServer] : otherServers;
      ...
      for (const library of libraries) {
        ...
        for (const item of items) {
          for (const targetServer of targetServers) {
            // Resolve username for this target (same-server partner or cross-server mapped name)
            let targetUsername;
            if (targetServer.name.toLowerCase() === masterServer.name.toLowerCase()) {
              targetUsername = resolveSameServerPartner(user.Name, masterServer);
            } else {
              targetUsername = resolveTargetUsername(user.Name, targetServer);
            }
            if (targetUsername === null) {
              totalSkipped++;
              continue;
            }
            const result = await syncItemToTarget(item, targetUsername, targetServer);
