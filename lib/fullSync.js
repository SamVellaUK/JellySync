...
const { shouldSyncUser, resolveTargetUsername } = require('./config');

/**
 * Sync a single item from master to target server
 * targetUsername: the already-resolved username to use on the target server
 */
...
async function performFullSync(config) {
  ...
  // Get target servers (all except master, plus master itself if it has any self-user-mappings)
  const otherServers = config.subscribers.filter(s => s.name.toLowerCase() !== config.masterServer.toLowerCase());
  const masterHasSelfMapping = masterServer.userMap &&
    Object.entries(masterServer.userMap).some(([src, tgt]) => src !== tgt);
      ...
      for (const library of libraries) {
        ...
        for (const item of items) {
          for (const targetServer of targetServers) {
            const targetUsername = resolveTargetUsername(user.Name, targetServer);
            if (targetUsername === null) {
              // User not in this target's userMap — skip
              totalSkipped++;
              continue;
            }
