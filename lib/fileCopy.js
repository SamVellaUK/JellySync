 ...
 * Requires a userId — picks the first available user on the master server.
 */
async function getItemPath(masterServer, itemId) {
  try {
    const users = await makeRequest(`${masterServer.url}/Users?api_key=${masterServer.apiKey}`);
    if (!users || users.length === 0) {
      console.error(`[COPY] No users found on master server to look up item path`);
      return null;
    }
    // Prefer an admin user — they have access to all library items
    const admin = users.find(u => u.Policy && u.Policy.IsAdministrator) || users[0];
    const item = await makeRequest(`${masterServer.url}/Users/${admin.Id}/Items/${itemId}?Fields=Path&api_key=${masterServer.apiKey}`);
...
function handleFileCopy(webhookData, fileCopyConfig, masterServer, targetServers) {
  ...
  (async () => {
    try {
      ...
      // Copy to each target server that has a destRoot configured
      const copyTargets = targetServers.filter(s => s.destRoot);
      if (copyTargets.length === 0) {
        console.warn('[COPY] No target servers have destRoot configured — skipping copy');
        return;
      }

      for (const server of copyTargets) {
        const serverDestRoot = server.destRoot.replace(/\/+$/, '');
        const destPath = path.join(serverDestRoot, relPath);
        console.log(`[COPY] Copying to ${server.name}: ${sourcePath} -> ${destPath}`);
        try {
          await fs.promises.mkdir(path.dirname(destPath), { recursive: true });
          await fs.promises.copyFile(sourcePath, destPath);
          console.log(`[COPY] Copy to ${server.name} completed`);
          await triggerLibraryRefresh(server);
        } catch (copyError) {
          console.error(`[COPY] Copy to ${server.name} failed: ${copyError.message}`);
        }
      }
    } catch (error) {
