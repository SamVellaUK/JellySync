...
const { shouldSyncUser, getOtherServers, resolveTargetUsername } = require('./config');
...
async function processWebhook(webhookData, config) {
  ...
  // Build target list: all other servers, plus source server itself if it has a self-user-mapping
  // (e.g. "LivingRoom" → "Bedroom" on the same server — allows same-server cross-user sync)
  const otherServers = getOtherServers(ServerName, config.subscribers);
  const selfMapped = sourceServer.userMap &&
    sourceServer.userMap[NotificationUsername] &&
    sourceServer.userMap[NotificationUsername] !== NotificationUsername;
  const targetServers = selfMapped ? [...otherServers, sourceServer] : otherServers;
  ...
  for (const targetServer of targetServers) {
    ...
    // Resolve the username to use on this target (handles userMap)
    const targetUsername = resolveTargetUsername(NotificationUsername, targetServer);
    if (targetUsername === null) {
      console.log(`[SYNC] User ${NotificationUsername} not in userMap for ${targetServer.name}, skipping`);
      continue;
    }
